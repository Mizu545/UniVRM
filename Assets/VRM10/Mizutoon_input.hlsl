...
#include "./Mizutoon_pipeline.hlsl"

// Textures
MIZUTOON_DECLARE_TEX2D(_MainTex);
MIZUTOON_DECLARE_TEX2D(_LightTex);
MIZUTOON_DECLARE_TEX2D(_ShadeTex);
MIZUTOON_DECLARE_TEX2D(_BumpMap);
MIZUTOON_DECLARE_TEX2D(_ShadingShiftTex);
MIZUTOON_DECLARE_TEX2D(_EmissionMap);
MIZUTOON_DECLARE_TEX2D(_MatcapTex);
MIZUTOON_DECLARE_TEX2D(_RimTex);
MIZUTOON_DECLARE_TEX2D(_OutlineWidthTex);
// NOTE: "tex2d() * _Time.y" returns mediump value if sampler is half precision in Android VR platform
MIZUTOON_DECLARE_TEX2D_FLOAT(_UvAnimMaskTex);
